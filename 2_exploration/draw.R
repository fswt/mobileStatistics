...
test_plot <- function(subset_one, subset_two, pic_name, plot_name, x_lab, y_lab){
  plot(subset_one$timestamp,subset_one$magnitude,type="l",col="red", main=plot_name, xlab = x_lab,ylab = y_lab)
...
test_hist <- function(subset_one, pic_name){
  ...
  curve(dnorm(x,mean=mean(subset_one$magnitude), sd=sd(subset_one$magnitude)),add = TRUE)
  path= paste("./graphs/",pic_name,".png")
  dev.copy(png,path)
  dev.off()
}
